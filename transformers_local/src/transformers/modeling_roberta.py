...
from NeuronalTransformerLayers.NeuronalTransformer import NeuronalTransformer
class NeuralRobertaModel(RobertaPreTrainedModel):
    """
    ...
    # Copied from transformers.modeling_bert.BertModel.__init__ with Bert->Roberta
    def __init__(self, roberta_config, neuron_config, add_pooling_layer=True):
        super().__init__(roberta_config, neuron_config)
        self.config = roberta_config

        self.embeddings = RobertaEmbeddings(roberta_config)
        self.encoder = NeuronalTransformer(roberta_config, neuron_config)

        self.pooler = RobertaPooler(roberta_config) if add_pooling_layer else None
...
@add_start_docstrings("""RoBERTa Model with a a neural transformer body""", ROBERTA_START_DOCSTRING)
class NeuralRobertaForMaskedLM(RobertaPreTrainedModel):
    authorized_missing_keys = [r"position_ids", r"predictions.decoder.bias"]
    authorized_unexpected_keys = [r"pooler"]

    def __init__(self, roberta_config, neural_config):
        super().__init__(roberta_config)

        if roberta_config.is_decoder:
            logger.warning(
                "If you want to use `RobertaForMaskedLM` make sure `config.is_decoder=False` for "
                "bi-directional self-attention."
            )
