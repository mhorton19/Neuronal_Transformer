...
class NeuronalLayer(nn.Module):
    def __init__(self, roberta_config, neuron_config):
        ...
        self.gen_neuron_outputs = GenNeuronOutputs(neuron_config)
        ...
        self.num_neurons = neuron_config.num_neurons
    ...
    def forward(self, hidden_states, prev_neuron_outputs=None, neuron_bank_connectivity_sub=0, output_connectivity_sub=0):
        ...
        if prev_neuron_outputs == None:
            prev_neuron_outputs = self.gen_neuron_outputs(torch.normal(0, 1, size=(hidden_states.shape[0], self.num_neurons, self.values_len * self.num_heads), device=hidden_states.device))

        neuron_keys, neuron_queries, neuron_values = prev_neuron_outputs
        keys = torch.cat((input_keys, neuron_keys), dim=1)
        values = torch.cat((input_values, neuron_values), dim=1)
